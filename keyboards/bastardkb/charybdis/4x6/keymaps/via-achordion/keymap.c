...
typedef enum charybdis_hand {
     CHARYBDIS_LEFT,
     CHARYBDIS_LEFT_THUMB,
     CHARYBDIS_RIGHT,
     CHARYBDIS_RIGHT_THUMB,

 } charybdis_hand;

charybdis_hand charybdis_key_hand(keypos_t pos) {
     if (pos.row < 0x04) {
         return CHARYBDIS_LEFT;
     } else if (pos.row == 0x04) {
         return CHARYBDIS_LEFT_THUMB;
     } else if (pos.row < 0x08) {
         return CHARYBDIS_RIGHT;
     } else { // pos.row == 0x09
         return CHARYBDIS_RIGHT_THUMB;
     }
 }

bool achordion_chord(uint16_t tap_hold_keycode, keyrecord_t* tap_hold_record, uint16_t other_keycode, keyrecord_t* other_record) {
    return charybdis_key_hand(tap_hold_record->event.key) != charybdis_key_hand(other_record->event.key);
}
